...
-- This version properly handles the signup flow:
-- 1. When user signs up, full_name and role are passed in metadata
-- 2. The trigger captures these from metadata and creates the profile
-- 3. The Flutter app only calls signUp() - doesn't create profile
-- 4. After signup, user is signed out and redirected to login
-- 5. On login, the profile already exists with all data saved
...
-- ============================================================
-- TRIGGER FUNCTIONS (ONLY FOR UPDATE)
-- ============================================================

-- Update timestamp function (only for UPDATE)
...
-- ============================================================
-- TRIGGERS (ONLY ON UPDATE, NOT INSERT)
-- ============================================================
...
-- ============================================================
-- Done. Database is properly configured.
-- All tables have updated_at with DEFAULT NOW()
-- Triggers only run on UPDATE to update the updated_at timestamp
--
-- KEY FIX FOR SIGNUP:
-- The handle_new_user trigger now extracts full_name and role
-- from user_metadata that is sent during signUp().
-- This means Flutter app just calls signUp() with metadata,
-- and the trigger automatically creates the complete profile.
-- ============================================================
